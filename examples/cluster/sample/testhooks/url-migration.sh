...
RESOURCE="release.helm.crossplane.io/url-migration-cluster"
NEW_VERSION="6.10.2"
...
SECRET=$(${KUBECTL} -n crossplane-system get secret -l "name=url-migration-cluster,owner=helm,release.helm.crossplane.io/url-hash" --sort-by=.metadata.creationTimestamp -o name | tail -n 1)
...
${KUBECTL} patch "${RESOURCE}" --type=merge -p "{\"spec\":{\"forProvider\":{\"chart\":{\"url\":\"${NEW_URL}\",\"version\":\"${NEW_VERSION}\"}}}}"
