...
type ChartSpec struct {
	...
	// When URL is set, Version is not late initialized and changing it does not trigger an
	// upgrade: change the URL tag or Digest instead. A version embedded in an OCI URL takes
	// effect and a conflicting Version is rejected at deploy time; a bare OCI URL deploys
	// Version. Avoid floating tags (e.g. :latest, or no tag and no Version): a new chart
	...
	PullSecretRef xpv2.LocalSecretReference `json:"pullSecretRef,omitempty"`
}
...
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,managed,helm}
type Release struct {
