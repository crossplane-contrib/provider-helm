					...
					func(release *v1beta1.Release) {
						release.Spec.ForProvider.CABundle = &v1beta1.ValueFromSource{
							SecretKeyRef: &v1beta1.DataKeySelector{
								NamespacedName: v1beta1.NamespacedName{Name: "missing-ca-secret", Namespace: "default"},
							},
						}
						now := metav1.Now()
...
func Test_withRelease(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"Defaults": {
			// Ownership and labels are not Connect-time arguments: they are
			// decided per deploy by deployOptions.
			args: args{
				cr: helmRelease(func(r *v1beta1.Release) {
					r.Spec.ForProvider.TakeOwnership = true
				}),
